...
TEST_F(DeviceTestSuite, setAppManagerMountsApp) {
    auto* app = new MockStateMachine(1);
    device->setAppManager(app);
    ASSERT_EQ(app->mountedCount, 1);
    ASSERT_EQ(app->loopCount, 0);
}

TEST_F(DeviceTestSuite, loopCallsAppStateLoop) {
    auto* app = new MockStateMachine(1);
    device->setAppManager(app);
    device->loop();
    device->loop();
    device->loop();
    ASSERT_EQ(app->loopCount, 3);
}

TEST_F(DeviceTestSuite, loopWithNoAppManagerDoesNotCrash) {
    device->loop();
    device->loop();
    SUCCEED();
}
