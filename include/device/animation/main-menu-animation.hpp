...
public:
    MainMenuAnimation() : AnimationBase() {}
...
private:
    void animateRecessLights() {
        for (uint8_t i = 0; i < NUM_RECESS_LIGHTS; i++) {
            currentState_.setRecessLight(i, mainMenuRecessColors[i], RECESS_BRIGHTNESS);
        }
        ...
        for (auto& s : sparkles_) {
            ...
            currentState_.setRecessLight(s.ledIdx, interpolateColor(base, peak, easedT), RECESS_BRIGHTNESS);
        }
    }
    ...
    void animateFinLights() {
        ...
        finOffset_ = std::min((uint16_t)(finOffset_ + FIN_STEP), FIN_CYCLE);

        for (uint8_t i = 0; i < NUM_FIN_LIGHTS; i++) {
            // No modulo — virtualPos grows past FIN_CYCLE, driving LEDs into the black zone
            uint16_t virtualPos = (uint16_t)(i * 256) + finOffset_;
            uint8_t  paletteIdx = virtualPos / 256;
            uint8_t  fraction   = virtualPos % 256;

            LEDColor color;
            if (paletteIdx < NUM_FIN_PALETTE) {
                // Blend toward the next palette entry, or toward black after the last one
            ...
            currentState_.setFinLight(i, color, FIN_BRIGHTNESS);
        }
    ...
    static constexpr uint8_t  RECESS_BRIGHTNESS = 87;
    static constexpr uint8_t  FIN_BRIGHTNESS    = 87;
