...
public:
    ...
    /// chain: ~20 hops at the 20ms cadence, plus a quarter again of margin for
    /// the receiving device's exec()-to-parse gap, which nothing caps. Not sized
    /// off MAX_CHAIN_MEMBERS: that is a roster-array cap set past any real chain,
    /// and a window sized off it would wait out a topology nobody cables.
    /// Hardware-proven at this value; windows that wait out such a walk derive
    /// from it so they move together, and moving it wants a rig sweep.
    static constexpr unsigned long CHAIN_PROPAGATION_MS = 500;
